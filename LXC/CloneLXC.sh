# Usage:
# ./CloneLXCs.sh <source_ct_id> <base_ct_name> <start_ct_id> <num_cts> <start_ip/cidr> [gateway] [pool_name]
#
# Arguments:
...
#
# Example:
#   ./CloneLXCs.sh 110 Ubuntu-2C-20GB 400 30 192.168.1.50/24 192.168.1.1 PoolName
#   ./CloneLXCs.sh 110 Ubuntu-2C-20GB 400 30 192.168.1.50/24 # Without specifying a gateway or pool

# Check if the minimum required parameters are provided
if [ "$#" -lt 5 ]; then
    echo "Usage: $0 <source_ct_id> <base_ct_name> <start_ct_id> <num_cts> <start_ip/cidr> [gateway] [pool_name]"
    exit 1
fi
...
for (( i=0; i<$NUM_CTS; i++ )); do
    ...
    # Set the static IP, subnet mask, and gateway
    pct set $TARGET_CT_ID -net0 name=eth0,ip=${NEW_IP}/${SUBNET_MASK},gw=${GATEWAY}
